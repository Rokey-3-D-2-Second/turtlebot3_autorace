...
class StableWaypointFollower(Node):
    def __init__(self):
        ...
        self.get_logger().info('StableWaypointFollower 노드 시작')
        self.leader_sub = self.create_subscription(Odometry, '/odom', self.leader_odom_cb, 10)
        self.follower_sub = self.create_subscription(Odometry, '/tb3_0/odom', self.follower_odom_cb, 10)
    ...
    def leader_odom_cb(self, msg):
        self.get_logger().info('리더 오도메트리 콜백 호출')
    ...
    def follower_odom_cb(self, msg):
        self.get_logger().info('팔로워 오도메트리 콜백 호출')
    ...
    def update(self):
        self.get_logger().info(
            f'update 호출: follower_pose={self.follower_pose is not None}, '
            f'waypoints={len(self.waypoints)}, leader_pose={self.leader_pose is not None}'
        )
        if self.follower_pose is None or len(self.waypoints) < 2 or self.leader_pose is None:
            self.get_logger().info('조건 미충족으로 update 종료')
            return
        ...
        if self.start_time is None:
            ...
            self.get_logger().info(f'추종 시작 시간 설정: {self.start_time}')
        ...
        if target_idx >= len(self.waypoints):
            self.get_logger().info('모든 waypoint 도달, 정지 명령 발행')
        ...
        self.get_logger().info(f'목표 waypoint 인덱스: {target_idx}, 거리: {distance:.3f}, 각도 차이: {angle_diff:.3f}')
        ...
        if (distance < self.WAYPOINT_DIST_THRESHOLD and abs(angle_diff) < self.WAYPOINT_ANGLE_THRESHOLD):
            ...
            self.get_logger().info(f'다음 waypoint로 이동: {self.current_idx}')
            if self.current_idx == len(self.waypoints) - 1:
                self.get_logger().info('마지막 waypoint 도달, 정지 명령 발행')
                self.cmd_pub.publish(Twist())
                return
        ...
        self.get_logger().info(
            f'퍼블리시할 cmd_vel: linear.x={cmd.linear.x:.3f}, angular.z={cmd.angular.z:.3f}'
        )
