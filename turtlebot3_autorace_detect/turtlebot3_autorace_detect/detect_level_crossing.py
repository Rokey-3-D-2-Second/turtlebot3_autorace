...
class DetectLevelNode(Node):
    ...
    def __init__(self):
        ...
        if self.pub_image_type == 'compressed':
            ...
            if self.is_calibration_mode:
                self.pub_image_color_filtered = self.create_publisher(Image, '/detect/image_output_sub1', 10)
    ...
    def find_rect_of_level(self, mask):
            ...
            if dx == 0 or abs(slope) > 2.0:
                ...
                if is_rects_linear or is_rects_dist_equal:
                    ...
                    if distance_bar2car > 1.0:
                        is_level_detected = True  # 레벨 감지 (감속 신호)
        ...
        # 블롭 개수가 1개 이하라면 레벨이 열려있다고 판단
        elif len(keypts) <= 1:
            is_level_opened = True
            self.stop_bar_state = 'go'

        # 최종 결과 이미지 퍼블리시 (compressed / raw 타입에 따라)
        if self.pub_image_type == 'compressed':
